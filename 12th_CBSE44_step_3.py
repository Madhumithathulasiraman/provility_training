...
class Step_3(Scene):
    def construct(self):
        ...
        triangle = Polygon(B, A, C, color=BLUE, stroke_width=4).shift(LEFT*2).set_z_index(10)

        AB = Line(B, A).shift(LEFT*2)
        AC = Line(A, C).shift(LEFT*2)
        BC = Line(B, C).shift(LEFT*2)

        right_angle = RightAngle(AB, AC, length=0.5, color=BLACK, quadrant=(-1, 1))
        tick_h = VGroup(Line(LEFT*0.2, RIGHT*0.2), Line(LEFT*0.2, RIGHT*0.2).shift(DOWN*0.15)).move_to(AB.get_center()).set_color(BLACK).rotate(1*DEGREES).set_z_index(10)
        tick_r = VGroup(Line(UP*0.2, DOWN*0.2), Line(UP*0.2, DOWN*0.2).shift(RIGHT*0.15)).rotate(-1*DEGREES).move_to(AC.get_center()).set_color(BLACK).set_z_index(10)

        angle_bottom = Arc(radius=1.0,start_angle=91*DEGREES,angle=-41*DEGREES,arc_center=B,color="#2A9C07").shift(LEFT*2)
