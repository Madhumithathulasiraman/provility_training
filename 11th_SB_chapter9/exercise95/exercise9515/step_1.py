...
class DiscontinuityDiagram(Scene):
    def construct(self):
        self.camera.background_color = WHITE

       
        axes = Axes(x_range=[-3, 5, 1],y_range=[-3, 10, 1],x_length=8,y_length=6,axis_config={"color": GREEN,"stroke_width":4,"include_ticks":False},tips=False)
        axes.x_axis.add_tip()
        ...
        x0 = 2
        left_curve = axes.plot(lambda x: 3 - 0.1*(x+1)**2,x_range=[-3, x0],color="#2A27F5",stroke_width=5).shift(UP*1.8).set_z_index(10)
        right_curve = axes.plot(lambda x: 1.3*(x-x0)**2 + 2.2,x_range=[x0, 4],color="#F58727",stroke_width=4)
        ...
        filled_dot = Dot(axes.c2p(x0, 6),color=BLUE).scale(1.2).set_z_index(10)
        ...
        arrow = Arrow(start=LEFT * 2,end=RIGHT * 2,color="#2A27F5",buff=0).scale(0.8).shift(LEFT*1+UP*0.7)
        arrow2= Arrow(start=RIGHT * 2,end=LEFT * 2,color="#F58727",buff=0).scale(0.65).shift(RIGHT*2.6+DOWN*1)
        ...
        eq1 = MathTex(r"\text{There exists a gap between } x_0^{+} \text{ and } x_0^{-}", color=BLACK).scale(0.5)
        eq2 = MathTex(r"(ie)\lim_{x \to x_0} f(x) \neq f(x_0)", color=BLACK).scale(0.5)
