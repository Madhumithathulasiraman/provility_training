...
class ExerciseAnswer9515b(Scene):
    def construct(self):
        ...
        axes=Axes(x_range=[-1.5, 8, 1],y_range=[-1.9, 6, 1],x_length=10,y_length=6,axis_config={"color":GREEN, "stroke_width":4,"include_ticks":False},tips=False)
        ...
        dot2.move_to(axes.c2p(x0,x0)).shift(DOWN*1.01)

        x0_label = MathTex("x_0", color=BLACK)
        x0_label.next_to(axes.c2p(x0, 0),DOWN,buff=0.2)

        self.add(axes, x_label1, y_label1,curve,v_line,open_dot,dot,dot2,x_label,y_label,zero,x0_label)

        arrow = Arrow(start=LEFT * 1,end=RIGHT * 1,color=RED,buff=0,tip_length=0.3).scale(0.65).shift(RIGHT*1+DOWN*0.3)
        arrow2= Arrow(start=RIGHT * 1,end=LEFT * 1,color=RED,buff=0,tip_length=0.3).scale(0.65).shift(RIGHT*2.6+UP*-0.28)
        label = MathTex("x_0^{+}", color=BLACK)
        label.next_to(arrow.get_center(),LEFT*-0+DOWN*0.5).scale(0.8)
        label1 = MathTex("x_0^{-}", color=BLACK)
        label1.next_to(arrow2.get_center(),DOWN*0.9).scale(0.8)
        self.add(arrow,arrow2,label, label1)
        eq1 = MathTex( r"f(x) \neq f(x_0)", color=BLACK).scale(0.5)
        eq2 = MathTex( r"\lim_{x \to x_0^{+}} f(x) \neq \lim_{x \to x_0^{-}} f(x)", color=BLACK).scale(0.5)
        ...
        full_box.scale(0.9)  
        full_box.shift(UP*2.5+ RIGHT*0.5)
        self.add(full_box)
        arrow_up = Arrow(DOWN,UP,buff=0,color=BLACK,stroke_width=4,tip_length=0.2).scale(0.5).shift(UP*1.3)
        self.add(arrow_up)