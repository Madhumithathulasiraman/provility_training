...
class ExerciseAnswer9515d(Scene):
    def construct(self):
        ...
        left_curve = axes.plot(lambda x: 0.06*(x+2)**2 + 2,x_range=[-2.5,x0],color="#0BC9F4")
        right_curve = axes.plot(lambda x: 0.4*(x-x0)**2,x_range=[x0,4],color="#27F52E").shift(UP*1.45)
        circle = Circle(radius=0.08,color="#3127F5")
        circle.move_to(axes.c2p(x0,3))
        circle.set_fill(WHITE,opacity=1).set_z_index(10)
        dot = Dot(axes.c2p(x0,2),color="#C109C8").set_z_index(10)
        ...
        self.add(axes, brace)
        arrow = Arrow(start=LEFT *1,end=RIGHT *1,color="#0BC9F4",buff=0,tip_length=0.2).scale(0.5).shift(LEFT*0.1+UP*1)
        arrow2= Arrow(start=RIGHT *1,end=LEFT *1,color="#27F52E",buff=0,tip_length=0.2).scale(0.5).shift(RIGHT*1.6+UP*0.1)
