...
class ExerciseAnswer9515d(Scene):
    def construct(self):
        ...
        right_curve = axes.plot(lambda x: 0.4*(x-x0)**2,x_range=[x0,4],color="#1009C8").shift(UP*1.45)
        ...
        vertical = Line(axes.c2p(x0,0),axes.c2p(x0,2.95),color="#9C2307")
        ...
        arrow2= Arrow(start=RIGHT *1,end=LEFT *1,color="#1009C8",buff=0,tip_length=0.2).scale(0.5).shift(RIGHT*1.6+UP*0.1)
        ...
        self.add(full_box)
        arrow = Arrow(DOWN,UP,buff=0,color=BLACK,stroke_width=2,tip_length=0.10).scale(0.48).shift(RIGHT*1.35+UP*1.23)
        self.add(arrow)