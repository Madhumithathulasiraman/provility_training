...
class Exercise919Answer(Scene):
    def construct(self):
        ...
        #Axes
        axes=Axes(x_range=[-3,5.9,1],y_range=[-2,5.9,1],x_length=10,y_length=10,axis_config={"color":BLACK,"stroke_width":3.5},tips=False)
        ...
        for tick in axes.x_axis.get_tick_marks():
            if tick.get_center()[0]<-4:
                tick.set_opacity(0)
        tick = axes.y_axis.get_tick(1)
        axes.y_axis.add(tick)
        ...
        labels=axes.get_axis_labels(x_label=MathTex("x",color=BLACK),y_label=MathTex("y", color=BLACK)).shift(DOWN*0.2+LEFT*0)
        x1_label=MathTex("x'", color=BLACK).next_to(axes.x_axis.get_start(),LEFT*0.05+UP*0.2)
        y1_label=MathTex("y'", color=BLACK).next_to(axes.y_axis.get_start(),DOWN*0.05+RIGHT*1.2+UP*0.1)
        ...
        dashed_line=DashedLine(start=axes.c2p(2,2),end=axes.c2p(2,0),color="#69DB43",stroke_width=6)
        dashed_line2=DashedLine(start=axes.c2p(0,2),end=axes.c2p(2,2),color="#69DB43",stroke_width=6)
        self.add(dashed_line,dashed_line2)
        
        arrow_1=Arrow(LEFT*3,RIGHT*3,color="#D2DB24",stroke_width=4,tip_length=0.3).shift(DOWN*3.3+LEFT*1.7).scale(0.8)
        arrow_2=Arrow(RIGHT*3,LEFT*3,color="#D2DB24",stroke_width=4,tip_length=0.3).shift(DOWN*3.3+RIGHT*2.9).scale(0.8)
        arrow_3=Arrow(DOWN*2,UP*3,color="#D2DB24",stroke_width=4,tip_length=0.3).shift(DOWN*4.5+RIGHT*0.6).scale(0.3)
        ...
        self.add(zero_label)
        self.add(axes,tick,labels,label_eq,labels_left,labels_right,labels_up)
