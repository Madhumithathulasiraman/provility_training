    ...
    def construct(self):
        ...
        for t in np.arange(-6, 6, spacing):
            ...
            for s1, s2 in merged:
                ...
                hatch.add(line)
        label1 = Text("A", color=BLACK, font_size=40).move_to(circle_1).shift(UP*2.7)
        label2 = Text("B", color=BLACK, font_size=40).move_to(circle_2).shift(UP*2.7)
        left_value = Text("e, i", color=BLACK).move_to(circle_1.get_center() + LEFT*0.5)
        left_value2 = Text("o, u", color=BLACK).move_to(circle_1.get_center() + LEFT*0.5+DOWN*1) 
        middle_value = Text("a", color=BLACK).shift(LEFT*0.1) 
        right_value = Text("b", color=BLACK).move_to(circle_2.get_center() + RIGHT*0.5)
        right_value2 = Text("c", color=BLACK).move_to(circle_2.get_center() + RIGHT*0.5+DOWN*1)
        outline1 = circle_1.copy().set_fill(opacity=0)
        outline2 = circle_2.copy().set_fill(opacity=0)

        self.add(rectangle,u_label,hatch,outline1, outline2,label1, label2,left_value,left_value2,middle_value,right_value,right_value2)