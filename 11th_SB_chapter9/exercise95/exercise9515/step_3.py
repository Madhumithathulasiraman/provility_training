...
class ExerciseAnswer9115c(Scene):
    def construct(self):
        ...
        axes = Axes(x_range=[-2.2, 5.5, 1],y_range=[-3.5,4, 1],x_length=10,y_length=7,axis_config={"color":"#079C1E","stroke_width":4,"include_ticks":False},tips=False)
        axes.x_axis.add_tip()
        axes.y_axis.add_tip()
        ...
        graph_left = axes.plot(func,x_range=[-1.9,2.68],color="#F52742",stroke_width=5).shift(DOWN*0.1)
        start_point = graph_left.get_start()
        next_point = graph_left.point_from_proportion(0.2)
        tip_start = Arrow(next_point,start_point,buff=0,stroke_width=5,tip_length=0.2,color="#F52742",max_tip_length_to_length_ratio=2).shift(LEFT*0.05)
        self.add(tip_start)
        graph_right = axes.plot(func,x_range=[3.28,5.5],color="#F52742",stroke_width=5)
        ...
        arrow_down = Arrow(UP,DOWN,buff=0,color="#6F27F5",stroke_width=4,tip_length=0.2).scale(0.4).shift(DOWN*2.9+RIGHT*1)
        ...
        arrow = Arrow(start=LEFT * 1,end=RIGHT * 1,color="#C82009",buff=0,tip_length=0.3).scale(0.65).shift(RIGHT*1+UP*0.2)
        ...
        label1.next_to(arrow2.get_center(),UP*0.5).scale(0.8)
        self.add(arrow,arrow2,label, label1)

        eq1 = MathTex( r"f(x) \text{ does not exist}", color=BLACK).scale(0.5)
        eq2=MathTex(r"\text{Not continuous at } x = x_0",color=BLACK).scale(0.5)
        ...
        full_box.scale(0.9)  
        full_box.shift(UP*1+ RIGHT*5.7)
        self.add(full_box)
        arrow_eq= Arrow(RIGHT,DOWN,buff=0,color=BLACK,stroke_width=4,tip_length=0.2).scale(1.9)
