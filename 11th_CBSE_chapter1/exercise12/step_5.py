...
class FlowerGraph(Scene):
    def construct(self):
        ...
        axes = Axes(
            x_range=[-4, 4, 1],
            y_range=[-4, 4, 1],
            axis_config={"include_tip":False,"color":BLACK,"include_ticks":False,"stroke_width":3},
        )
        ...
        # Generate many circles passing through origin
        for angle in np.linspace(0, 2*PI, 5):  # more = better "infinite" feel
            r = 1  # distance from origin to center
            cx = r * np.cos(angle)
            cy = r * np.sin(angle)
